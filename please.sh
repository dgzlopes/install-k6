...
###############################################################################
# 1. Define color variables and helper functions
###############################################################################
Color_Off=''
Red=''
Green=''
...
install_k6() {
  ...
  local WRAPPER_URL="https://install-k6.com/wrapper.sh"
  local WRAPPER_EXE="$BIN_DIR/k6"
  local RUNTIME_DIR="$BIN_DIR/runtime"
  local REAL_EXE="$RUNTIME_DIR/k6"
  ...
  # Check if a k6 is already installed in the system
  local SYSTEM_K6_PATH
  SYSTEM_K6_PATH="$(command -v k6 || true)"
  if [[ -n "$SYSTEM_K6_PATH" && "$SYSTEM_K6_PATH" != "$WRAPPER_EXE" ]]; then
...
Installing in $BIN_DIR might override or conflict with the existing installation."
  ...
  if [[ -x "$WRAPPER_EXE" ]]; then
    # Attempt to get the current version by calling the wrapper
    local CURRENT_VERSION
    CURRENT_VERSION="$("$WRAPPER_EXE" version 2>/dev/null | awk '/^k6/ {print $2}')"
  ...
  # Locate the extracted k6 binary
  local TMP_BIN
  ...
  if [[ "$DISABLE_UPDATES_CHECK" == true ]]; then
    # If user doesn't want updates-check, just move the real binary into place as $BIN_DIR/k6
    mv "$TMP_BIN" "$WRAPPER_EXE"
    chmod +x "$WRAPPER_EXE"
  else
    # If we do want update checks, move the real binary to runtime/k6
    mv "$TMP_BIN" "$REAL_EXE"
    chmod +x "$REAL_EXE"

    # Download the wrapper script as ~/.k6/bin/k6
  ...
  if [[ "$UPDATE_MODE" == true ]]; then
    ...
    success "Successfully installed k6 $K6_VERSION at $WRAPPER_EXE"
  fi
}
...
echo
info_bold "Run 'k6' to confirm your installation and get started!"
